...
EOD;
    ...
    /*
    $results = array_merge($results, find_all_matches($results, $etos, 'etos'));
    $results = array_merge($results, find_all_matches($results, $arthro, 'arthro'));
    $results = array_merge($results, find_all_matches($results, $fek_tp, 'fek_tp'));
    $results = array_merge($results, find_all_matches($results, $ar_nomou, 'ar_nomou'));
    $results = array_merge($results, find_all_matches($results, $periptwsi, 'periptwsi'));
    $results = array_merge($results, find_all_matches($results, $paragrafos, 'paragrafos'));
    $results = array_merge($results, find_all_matches($results, $edafio, 'edafio'));
    
    $f = find_all_matches($results, $fek, 'fek');
    $results = array_merge($results, $f);
    $finals = array_merge($finals, $f);
    
    $f = find_all_matches($results, $nomos, 'nomos');
    $results = array_merge($results, $f);
    $finals = array_merge($finals, $f);
    
    $f = find_all_matches($results, $pd, 'pd');
    $results = array_merge($results, $f);
    $finals = array_merge($finals, $f);
    
    if (count($finals) > 0) {
      foreach($finals as $result) {
        echo <<<EOD
...
EOD;
        ...
        echo <<<EOD
<div title="{$cvalue}">{$cid}: ** {$cvalue} **</div>\n
EOD;
    }
    */
